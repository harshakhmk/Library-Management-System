...
def my_books(request):
    return render(request,'mybooks.html',{'books':Books.objects.filter(created_by_user_id=request.user)})

def newbook(request):
    if request.method =="GET":
        book_form = BookForm()
        return render(request,'newbook.html',{'form':book_form,'category':Category.objects.all()})
    else:
        book_form = BookForm(request.POST or None)
        if book_form.is_valid:
            book_obj = book_form.save(commit=False)

            category_obj,created = Category.objects.get_or_create(category=book_obj.category)
